...
function buildStepwiseTask(meta: SortMeta, stepsFn: (a: number[]) => number[][]): TaskData {
  ...
  return {
    ...
    answer: '',
    prompt: `Startarray: ${arrStr(initial)}. Sortieren Sie es mit ${meta.algoName}.`,
    inputHint: 'Die Lösung zeigt das Array nach jedem Schritt.',
    taskList: [`Sortiere das Array ${arrStr(initial)} mit ${meta.algoName}.`],
    steps,
    explanation: [
      `Startarray: ${arrStr(initial)}.`,
      meta.idea,
      `Führen Sie den Algorithmus schrittweise aus und notieren Sie das Array nach jeder Operation.`,
...
export function generateCountingSort(): TaskData {
  ...
  return {
    ...
    mathQuery: `\\text{Array: } ${arrStr(initial)}`,
    answer: '',
    prompt: `Startarray: ${arrStr(initial)}. Sortieren Sie es mit Countingsort (Werte 0…9).`,
    inputHint: 'Die Lösung zeigt das Zähl-Array und das sortierte Ergebnis.',
    taskList: [`Sortiere das Array ${arrStr(initial)} mit Countingsort (Werte 0…9).`],
    steps: [
...
export function generateBucketSort(): TaskData {
  ...
  return {
    ...
    mathQuery: `\\text{Array: } ${arrStr(initial)}`,
    answer: '',
    prompt: `Startarray: ${arrStr(initial)}. Sortieren Sie es mit Bucketsort (10 Buckets, Werte 0…99).`,
    inputHint: 'Die Lösung zeigt die Bucket-Inhalte und das sortierte Ergebnis.',
    taskList: [`Sortiere das Array ${arrStr(initial)} mit Bucketsort (10 Buckets, Werte 0…99).`],
    steps,
    explanation: [
