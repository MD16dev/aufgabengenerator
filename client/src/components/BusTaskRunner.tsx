...
export const BusTaskRunner: React.FC<BusTaskRunnerProps> = ({
  taskType,
  user,
  onSolved,
  onBackToSelector,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!task || !selectedOption || isLocked) return;
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 py-8 animate-fadeIn" id="bus-task-solver">
          ...
          <div>
                ...
                {task.choices?.map((option, idx) => {
                  ...
                  if (hasAnswered) {
                    if (isCorrectAnswer) {
                      ...
                      optionClass = 'border-rose-500 bg-rose-500/10 text-rose-600 dark:text-rose-400 font-semibold';
                    } else {
                      // Non-selected incorrect options are muted/disabled
                      optionClass = 'border-theme-border opacity-55 bg-theme-card/30';
                    }
                  } else if (isLocked) {
                    // Locked due to revealed explanation
                    if (isCorrectAnswer) {
                      optionClass = 'border-emerald-500 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-semibold';
                    } else {
                      optionClass = 'border-theme-border opacity-55 bg-theme-card/30';
            ...
            {status === 'incorrect' && (
              <div className="mt-4 p-3.5 bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400 rounded-xl text-sm font-bold flex items-start gap-2.5 animate-fadeIn">
                ...
                <div>
                  Leider nicht korrekt. Versuche es bei der nächsten Frage noch einmal oder lass dir die Lösung anzeigen!
                </div>
              </div>
            )}
            {status === 'correct' && (
